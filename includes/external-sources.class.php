    ...
    */
	...
	public function get_public_photos($search_user_id,$count){
		if(!empty($search_user_id)){
			...
			$count = $this->instagram_output_array($rsp->user->media->nodes,$count,$search_user_id);
	 ...
	 */
	private function instagram_output_array($photos,$count,$search_user_id){
		foreach ($photos as $photo) {
			if($count > 0){
				$count--;
				$stream = array();
				...
				$stream['title'] = $text;
				$stream['content'] = $text;
				$stream['date'] = date_i18n( get_option( 'date_format' ), strtotime( $photo->date ) ) ;
				$stream['date_modified'] = date_i18n( get_option( 'date_format' ), strtotime( $photo->date ) ) ;
